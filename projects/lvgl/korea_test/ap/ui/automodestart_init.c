...
#define TAG "[automodestart_init.c] "
...
void destroy_page_automodestart(bk_lv_ui_t *bk_ui)
{
    ...
    currentImageStep = 0;
    preRenderPageState[PAGE_AUTOMODESTART].isRendered = false;

    const uint32_t imageCount = preRenderPageConfig[PAGE_AUTOMODESTART].preRenderImageCount;
    for(uint32_t i = 0; i < imageCount; i++)
    {
        const preRenderImageInfo_t *imageInfo = &preRenderPageConfig[PAGE_AUTOMODESTART].preRenderImageInfo[i];
...
rendererFuncStatus_t init_page_automodestart_with_step(bk_lv_ui_t *bk_ui)
{
    ...
    if(preRenderPageState[PAGE_AUTOMODESTART].isRendered)
    {
    ...
    switch(currentStep)
    {
        case RENDER_STEP_CREATE_PAGE:
        {
            ...
            bk_printf(TAG "[RENDER][AUTOMODESTART] start tick=%lu\n", (unsigned long)renderStartTick);
            ...
            if(bk_ui->automodestart == NULL || !lv_obj_is_valid(bk_ui->automodestart))
            {
                bk_printf(TAG "[RENDER][AUTOMODESTART] CREATE_PAGE failed\n");
                return RENDERER_FUNC_FAILED;
            }
        ...
        {
            if(preRenderPageConfig[PAGE_AUTOMODESTART].backgroundImageAssetId != SHARED_IMAGE_NONE)
            {
                const sharedImageAssetId_t assetId =
                    preRenderPageConfig[PAGE_AUTOMODESTART].backgroundImageAssetId;
                if(set_shared_image_asset(bk_ui->automodestart_auto_bg, assetId) != RENDERER_FUNC_DONE)
                {
                    return RENDERER_FUNC_FAILED;
                }
            }
        ...
        case RENDER_STEP_CACHE_IMAGE:
        {
            ...
            if(currentImageStep < imageCount)
            {
                const preRenderImageInfo_t *imageInfo =
                    &preRenderPageConfig[PAGE_AUTOMODESTART].preRenderImageInfo[currentImageStep];
                const char *languageSuffix = imageInfo->hasLanguageVariant ?
                                             (settings_get_int("LANGUAGE") == 1 ? "_china" :
                                              settings_get_int("LANGUAGE") == 2 ? "_english" : "") : "";
                const char *degreeSuffix = imageInfo->hasDegreeVariant &&
                ...
                {
                    bk_printf(TAG "[PREWARM][AUTOMODESTART] image %lu/%lu failed: %s (%lu ms)\n",
                ...
                bk_printf(TAG "[PREWARM][AUTOMODESTART] image %lu/%lu done: %s (%lu ms)\n",
                          (unsigned long)(currentImageStep + 1),
                          (unsigned long)imageCount,
                          imagePath,
                          (unsigned long)lv_tick_elaps(imageStartTick));
        ...
        {
            /* init_page_automodestart() also attaches the page and control callbacks. */
        ...
        {
            bk_printf(TAG "[RENDER][AUTOMODESTART] done total=%lu ms\n",
                      (unsigned long)lv_tick_elaps(renderStartTick));

            currentStep = RENDER_STEP_CREATE_PAGE;
            currentImageStep = 0;
            renderStartTick = 0;
            preRenderPageState[PAGE_AUTOMODESTART].isRendered = true;
            return RENDERER_FUNC_DONE;
        }

        default:
        {
            bk_printf(TAG "[RENDER][AUTOMODESTART] invalid step=%lu\n",
