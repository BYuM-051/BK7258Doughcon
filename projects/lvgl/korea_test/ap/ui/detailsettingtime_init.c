...
#define TAG "[detailsettingtime_init.c] "
...
void destroy_page_detailsettingtime(bk_lv_ui_t *bk_ui)
{
    ...
    currentImageStep = 0;
    preRenderPageState[PAGE_DETAILSETTINGTIME].isRendered = false;

    const uint32_t imageCount = preRenderPageConfig[PAGE_DETAILSETTINGTIME].preRenderImageCount;
    for(uint32_t i = 0; i < imageCount; i++)
    {
        const preRenderImageInfo_t *imageInfo = &preRenderPageConfig[PAGE_DETAILSETTINGTIME].preRenderImageInfo[i];
...
create_children:
    ...
    lv_obj_add_event_cb(bk_ui->detailsettingtime_backbt, detailsettingtime_backbt_event_cb, LV_EVENT_PRESSED, NULL);
    ...
    lv_obj_add_event_cb(bk_ui->detailsettingtime_settingbt1, detailsettingtime_settingbt1_event_cb, LV_EVENT_PRESSED, NULL);
    ...
    lv_obj_add_event_cb(bk_ui->detailsettingtime_settingbt2, detailsettingtime_settingbt2_event_cb, LV_EVENT_PRESSED, NULL);
    ...
    lv_obj_add_event_cb(bk_ui->detailsettingtime_settingbt3, detailsettingtime_settingbt3_event_cb, LV_EVENT_PRESSED, NULL);
    ...
    lv_obj_add_event_cb(bk_ui->detailsettingtime_settingbt4, detailsettingtime_settingbt4_event_cb, LV_EVENT_PRESSED, NULL);
    ...
    lv_obj_add_event_cb(bk_ui->detailsettingtime_settingbt5, detailsettingtime_settingbt5_event_cb, LV_EVENT_PRESSED, NULL);
...
#undef _TIME_ROLLER_STYLE
    ...
    lv_obj_add_event_cb(bk_ui->detailsettingtime_changebt, detailsettingtime_changebt_event_cb, LV_EVENT_PRESSED, NULL);
...
rendererFuncStatus_t init_page_detailsettingtime_with_step(bk_lv_ui_t *bk_ui)
{
    static uint32_t renderStartTick = 0;

    if(preRenderPageState[PAGE_DETAILSETTINGTIME].isRendered)
    {
    ...
    switch(currentStep)
    {
        case RENDER_STEP_CREATE_PAGE:
        {
            ...
            bk_printf(TAG "[RENDER][DETAILSETTINGTIME] start tick=%lu\n", (unsigned long)renderStartTick);
            ...
            if(bk_ui->detailsettingtime == NULL || !lv_obj_is_valid(bk_ui->detailsettingtime))
            {
                bk_printf(TAG "[RENDER][DETAILSETTINGTIME] CREATE_PAGE failed\n");
                return RENDERER_FUNC_FAILED;
            }

#if UI_PRENDERING_ENABLE
            lv_obj_add_flag(bk_ui->detailsettingtime, LV_OBJ_FLAG_HIDDEN);
#endif
        ...
        {
            if(preRenderPageConfig[PAGE_DETAILSETTINGTIME].backgroundImageAssetId != SHARED_IMAGE_NONE)
            {
                const sharedImageAssetId_t assetId =
                    preRenderPageConfig[PAGE_DETAILSETTINGTIME].backgroundImageAssetId;
                if(set_shared_image_asset(bk_ui->detailsettingtime_bg, assetId) != RENDERER_FUNC_DONE)
                {
                    return RENDERER_FUNC_FAILED;
                }
            }
        ...
        case RENDER_STEP_CACHE_IMAGE:
        {
            ...
            if(currentImageStep < imageCount)
            {
                const preRenderImageInfo_t *imageInfo =
                    &preRenderPageConfig[PAGE_DETAILSETTINGTIME].preRenderImageInfo[currentImageStep];
                const char *languageSuffix = imageInfo->hasLanguageVariant ?
                                             (settings_get_int("LANGUAGE") == 1 ? "_china" :
                                              settings_get_int("LANGUAGE") == 2 ? "_english" : "") : "";
                const char *degreeSuffix = imageInfo->hasDegreeVariant &&
                ...
                {
                    bk_printf(TAG "[PREWARM][DETAILSETTINGTIME] image %lu/%lu failed: %s (%lu ms)\n",
                ...
                bk_printf(TAG "[PREWARM][DETAILSETTINGTIME] image %lu/%lu done: %s (%lu ms)\n",
                          (unsigned long)(currentImageStep + 1),
                          (unsigned long)imageCount,
                          imagePath,
                          (unsigned long)lv_tick_elaps(imageStartTick));
        ...
        {
            /* init_page_detailsettingtime() also attaches the page and control callbacks. */
        ...
        {
            bk_printf(TAG "[RENDER][DETAILSETTINGTIME] done total=%lu ms\n",
                      (unsigned long)lv_tick_elaps(renderStartTick));

            currentStep = RENDER_STEP_CREATE_PAGE;
            currentImageStep = 0;
            renderStartTick = 0;
            preRenderPageState[PAGE_DETAILSETTINGTIME].isRendered = true;
            return RENDERER_FUNC_DONE;
        }

        default:
        {
            bk_printf(TAG "[RENDER][DETAILSETTINGTIME] invalid step=%lu\n",
