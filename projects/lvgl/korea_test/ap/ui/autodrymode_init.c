...
rendererFuncStatus_t init_page_autodrymode_with_step(bk_lv_ui_t *bk_ui)
{
    ...
    if(preRenderPageState[PAGE_AUTODRYMODE].isRendered)
    {
    ...
    switch (currentStep)
    {
        case RENDER_STEP_CREATE_PAGE :
        {
            ...
            bk_printf(TAG "[RENDER][AUTODRYMODE] start tick=%lu\n", (unsigned long)renderStartTick);
            ...
            bk_printf(TAG "[RENDER][AUTODRYMODE] CREATE_PAGE done elapsed=%lu ms\n",
        ...
        case RENDER_STEP_CREATE_CHILD :
        {
            ...
            bk_printf(TAG "[RENDER][AUTODRYMODE] CREATE_CHILD start\n");
            ...
            bk_printf(TAG "[RENDER][AUTODRYMODE] CREATE_CHILD done elapsed=%lu ms total=%lu ms\n",
                      (unsigned long)lv_tick_elaps(stepStartTick),
        ...
        {
            if(preRenderPageConfig[PAGE_AUTODRYMODE].backgroundImageAssetId != SHARED_IMAGE_NONE)
            {
                const sharedImageAssetId_t bgImageId = preRenderPageConfig[PAGE_AUTODRYMODE].backgroundImageAssetId;
        ...
        case RENDER_STEP_CACHE_IMAGE :
        {
            ...
            bk_printf(TAG "[PREWARM][AUTODRYMODE] CACHE_IMAGE start (%lu images)\n", (unsigned long)imageCount);
            ...
            if(currentImageStep < imageCount)
            {
                ...
                if(res != LV_RESULT_OK)
                {
                    bk_printf(TAG "[PREWARM][AUTODRYMODE] image %lu/%lu FAILED res=%d elapsed=%lu ms path=%s\n",
                              (unsigned long)(currentImageStep + 1),
                              (unsigned long)imageCount,
                              (int)res,
                ...
                bk_printf(TAG "[PREWARM][AUTODRYMODE] image %lu/%lu OK elapsed=%lu ms path=%s\n",
                          (unsigned long)(currentImageStep + 1),
                          (unsigned long)imageCount,
                          (unsigned long)imageElapsed,
                          imagePath);
            ...
            bk_printf(TAG "[PREWARM][AUTODRYMODE] all images done count=%lu total=%lu ms\n",
                      (unsigned long)imageCount,
                      (unsigned long)lv_tick_elaps(renderStartTick));
        ...
        case RENDER_STEP_ATTACH_EVENT :
        {
            ...
            bk_printf(TAG "[RENDER][AUTODRYMODE] ATTACH_EVENT start\n");
            ...
            lv_obj_add_event_cb(bk_ui->autodrymode_backbt, autodrymode_backbt_event_cb, LV_EVENT_PRESSED, NULL);
            lv_obj_add_event_cb(bk_ui->autodrymode_auto_dry_temp_bt, autodrymode_auto_dry_temp_bt_event_cb, LV_EVENT_PRESSED, NULL);
            lv_obj_add_event_cb(bk_ui->autodrymode_auto_dry_humidity_bt, autodrymode_auto_dry_humidity_bt_event_cb, LV_EVENT_PRESSED, NULL);
            lv_obj_add_event_cb(bk_ui->autodrymode_auto_dry_hour_bt, autodrymode_auto_dry_hour_bt_event_cb, LV_EVENT_PRESSED, NULL);
            lv_obj_add_event_cb(bk_ui->autodrymode_auto_dry_min_bt, autodrymode_auto_dry_min_bt_event_cb, LV_EVENT_PRESSED, NULL);
            lv_obj_add_event_cb(bk_ui->autodrymode_auto_dry_start, autodrymode_auto_dry_start_event_cb, LV_EVENT_PRESSED, NULL);

            bk_printf(TAG "[RENDER][AUTODRYMODE] ATTACH_EVENT done elapsed=%lu ms\n",
                      (unsigned long)lv_tick_elaps(stepStartTick));
        ...
        case RENDER_STEP_DONE :
        {
            ...
            bk_printf(TAG "[RENDER][AUTODRYMODE] DONE total=%lu ms\n", (unsigned long)totalElapsed);
        ...
        default :
        {
            bk_printf(TAG "[RENDER][AUTODRYMODE] INVALID STEP: %lu\n", (unsigned long)currentStep);
            return RENDERER_FUNC_FAILED;
        }
    }
}
