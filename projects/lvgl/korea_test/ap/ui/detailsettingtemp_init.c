...
#define TAG "[detailsettingtemp_init.c] "
#include "preRenderer.h"
...
void destroy_page_detailsettingtemp(bk_lv_ui_t *bk_ui)
{
    ...
    currentImageStep = 0;
    preRenderPageState[PAGE_DETAILSETTINGTEMP].isRendered = false;

    const uint32_t imageCount = preRenderPageConfig[PAGE_DETAILSETTINGTEMP].preRenderImageCount;
    for(uint32_t i = 0; i < imageCount; i++)
    {
        const preRenderImageInfo_t *imageInfo = &preRenderPageConfig[PAGE_DETAILSETTINGTEMP].preRenderImageInfo[i];
...
rendererFuncStatus_t init_page_detailsettingtemp_with_step(bk_lv_ui_t *bk_ui)
{
    static uint32_t renderStartTick = 0;

    if(preRenderPageState[PAGE_DETAILSETTINGTEMP].isRendered)
    {
    ...
    switch(currentStep)
    {
        case RENDER_STEP_CREATE_PAGE:
        {
            ...
            bk_printf(TAG "[RENDER][DETAILSETTINGTEMP] start tick=%lu\n", (unsigned long)renderStartTick);
            ...
            if(bk_ui->detailsettingtemp == NULL || !lv_obj_is_valid(bk_ui->detailsettingtemp))
            {
                bk_printf(TAG "[RENDER][DETAILSETTINGTEMP] CREATE_PAGE failed\n");
                return RENDERER_FUNC_FAILED;
            }

#if UI_PRENDERING_ENABLE
            lv_obj_add_flag(bk_ui->detailsettingtemp, LV_OBJ_FLAG_HIDDEN);
#endif
        ...
        {
            if(preRenderPageConfig[PAGE_DETAILSETTINGTEMP].backgroundImageAssetId != SHARED_IMAGE_NONE)
            {
                const sharedImageAssetId_t assetId =
                    preRenderPageConfig[PAGE_DETAILSETTINGTEMP].backgroundImageAssetId;
                if(set_shared_image_asset(bk_ui->detailsettingtemp_bg, assetId) != RENDERER_FUNC_DONE)
                {
                    return RENDERER_FUNC_FAILED;
                }
            }
        ...
        case RENDER_STEP_CACHE_IMAGE:
        {
            ...
            if(currentImageStep < imageCount)
            {
                const preRenderImageInfo_t *imageInfo =
                    &preRenderPageConfig[PAGE_DETAILSETTINGTEMP].preRenderImageInfo[currentImageStep];
                const char *languageSuffix = imageInfo->hasLanguageVariant ?
                                             (settings_get_int("LANGUAGE") == 1 ? "_china" :
                                              settings_get_int("LANGUAGE") == 2 ? "_english" : "") : "";
                const char *degreeSuffix = imageInfo->hasDegreeVariant &&
                ...
                {
                    bk_printf(TAG "[PREWARM][DETAILSETTINGTEMP] image %lu/%lu failed: %s (%lu ms)\n",
                ...
                bk_printf(TAG "[PREWARM][DETAILSETTINGTEMP] image %lu/%lu done: %s (%lu ms)\n",
                          (unsigned long)(currentImageStep + 1),
                          (unsigned long)imageCount,
                          imagePath,
                          (unsigned long)lv_tick_elaps(imageStartTick));
        ...
        {
            /* init_page_detailsettingtemp() also attaches the page and control callbacks. */
        ...
        {
            bk_printf(TAG "[RENDER][DETAILSETTINGTEMP] done total=%lu ms\n",
                      (unsigned long)lv_tick_elaps(renderStartTick));

            currentStep = RENDER_STEP_CREATE_PAGE;
            currentImageStep = 0;
            renderStartTick = 0;
            preRenderPageState[PAGE_DETAILSETTINGTEMP].isRendered = true;
            return RENDERER_FUNC_DONE;
        }

        default:
        {
            bk_printf(TAG "[RENDER][DETAILSETTINGTEMP] invalid step=%lu\n",
