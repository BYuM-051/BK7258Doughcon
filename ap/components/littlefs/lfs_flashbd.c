...
int lfs_rambd_read(const struct lfs_config *cfg, lfs_block_t block,
    ...
    // read data
	memcpy(buffer, (void *)(cfg->block_size*block+off+bd->start_addr), size);
...
int lfs_rambd_prog(const struct lfs_config *cfg, lfs_block_t block,
    ...
    // progflash data
    memcpy((void *)(cfg->block_size*block+off+bd->start_addr), buffer, size);
...
int lfs_rambd_erase(const struct lfs_config *cfg, lfs_block_t block) {
    ...
    // erase
    memset((void *)(cfg->block_size*block+bd->start_addr), 0xff, 4096);
