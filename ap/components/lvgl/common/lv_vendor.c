...
static u32 g_init_stack_size = (1024 * 24);
static beken_mutex_t g_disp_mutex = NULL;
static beken_semaphore_t lvgl_sem = NULL;
static beken_semaphore_t s_touch_sem = NULL;  /* touch wake-up */
...
bk_err_t lv_vendor_init(lv_vnd_config_t *config)
{
    ...
    rtos_init_semaphore_ex(&s_touch_sem, 1, 0);
...
#if CONFIG_LVGL_V8
            vendor_config.draw_pixel_size = config->width * config->height / 4;
#else
            vendor_config.draw_pixel_size = config->width * config->height / 4 * sizeof(bk_color_t);
#endif
        }
...
#endif
        rtos_get_semaphore(&s_touch_sem, sleep_time);  /* wakes on touch or timeout */
    }
...
/* Called from GT911 touch callback — wakes LVGL task immediately */
void lv_vendor_tp_notify(void *arg)
{
    (void)arg;
    if (s_touch_sem)
        rtos_set_semaphore(&s_touch_sem);
}
