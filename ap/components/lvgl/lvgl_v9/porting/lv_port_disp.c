...
/* per-frame flush timing — prints every frame that takes >200ms */
static uint32_t s_ft_start   = 0;
static uint32_t s_ft_dma     = 0;
static uint32_t s_ft_fbget   = 0;
static uint32_t s_ft_memcpy  = 0;
static uint32_t s_ft_frames  = 0;

/* tile area buffer so slow frames can dump per-tile info */
#define _TILE_BUF_MAX 8
static struct { int16_t x1, y1, x2, y2; } s_tiles[_TILE_BUF_MAX];
static int s_tile_n = 0;
...
static void lv_disp_flush_for_partial_mode(lv_display_t * disp_drv, const lv_area_t * area, uint8_t * px_map)
{
    ...
    if (CONFIG_LVGL_FRAME_BUFFER_NUM > 1) {
        /* capture start tick before DMA wait so flush_blk includes it */
        if (lv_new_frame_flag) {
            s_ft_start  = lv_tick_get();
            s_ft_dma    = 0;
            s_ft_fbget  = 0;
            s_ft_memcpy = 0;
            s_tile_n    = 0;
    ...
    /* buffer this tile's area for slow-frame reporting */
    if (s_tile_n < _TILE_BUF_MAX) {
        s_tiles[s_tile_n].x1 = (int16_t)area->x1;
        s_tiles[s_tile_n].y1 = (int16_t)area->y1;
        s_tiles[s_tile_n].x2 = (int16_t)area->x2;
        s_tiles[s_tile_n].y2 = (int16_t)area->y2;
        s_tile_n++;
    }
    ...
    if (lv_disp_flush_is_last(disp_drv)) {
        ...
        uint32_t _flush_ms = lv_tick_elaps(_t);
        lv_new_frame_flag = true;

        uint32_t _cbwait_ms = 0;
        uint32_t _dmastart_ms = 0;
        if (CONFIG_LVGL_FRAME_BUFFER_NUM > 1) {
            _t = lv_tick_get();
            if (copy_buf == NULL) {
                do {
            ...
            #endif
            _dmastart_ms = lv_tick_elaps(_t);
        }

        s_ft_frames++;
        {
            uint32_t flush_total = lv_tick_elaps(s_ft_start);
            if (flush_total > 200) {
                printf("[FLUSH#%lu] t=%lu dma=%lu fbget=%lu memcpy=%lu disp=%lu cbwait=%lu dma2=%lu tiles=%d\n",
                       s_ft_frames, flush_total,
                       s_ft_dma, s_ft_fbget, s_ft_memcpy,
                       _flush_ms, _cbwait_ms, _dmastart_ms, s_tile_n);
                for (int _i = 0; _i < s_tile_n; _i++) {
                    int _pw = s_tiles[_i].x2 - s_tiles[_i].x1 + 1;
                    int _ph = s_tiles[_i].y2 - s_tiles[_i].y1 + 1;
                    printf("  tile[%d]=(%d,%d-%d,%d) %dpx\n",
                           _i, s_tiles[_i].x1, s_tiles[_i].y1,
                           s_tiles[_i].x2, s_tiles[_i].y2, _pw * _ph);
                }
            }
        }
    }
