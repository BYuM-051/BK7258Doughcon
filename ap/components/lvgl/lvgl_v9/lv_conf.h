...
#define LV_INDEV_REFR_PERIOD  20    /*[ms]*/
...
#define LV_USE_OS   LV_OS_FREERTOS
    ...
    #define LV_OS_CUSTOM_INCLUDE <os/os.h>
#endif
...
/* Cap layer buffer at SIMPLE_BUF_SIZE so LVGL always uses chunked rendering.
 * Without this cap, a 1024x600 popup on lv_layer_top() triggers psram_malloc(1.2MB),
 * which fails on a fragmented heap and hits BK_ASSERT(0) before LVGL's NULL fallback. */
#define LV_DRAW_LAYER_MAX_MEMORY (24 * 1024)  /**< Cap at simple-buf size → chunked rendering [bytes]*/

/** Stack size of drawing thread.
 * NOTE: If FreeType or ThorVG is enabled, it is recommended to set it to 32KB or more.
 */
#define LV_DRAW_THREAD_STACK_SIZE    (12 * 1024)         /**< [bytes]*/
...
#define LV_CACHE_DEF_SIZE       (9* 1024 * 1024)  /* 1.5MB: password_popup(1.27MB) 수용 최솟값 + 여유 */
...
#define LV_USE_SYSMON   0
    ...
    /* LV_USE_PERF_MONITOR is enabled via CONFIG_LV_USE_PERF_MONITOR in sdkconfig.
     * Define _POS unconditionally so lv_conf_internal.h doesn't override with BOTTOM_RIGHT. */
