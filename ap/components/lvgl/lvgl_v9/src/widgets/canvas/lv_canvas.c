...
void lv_canvas_finish_layer(lv_obj_t * canvas, lv_layer_t * layer)
{
    ...
    /* lv_layer_top() 자식은 disp->screens[]에 없어서 lv_obj_get_display()가 NULL 반환.
     * NULL이면 layer_deinit/layer_free가 건너뛰어져 GPU 정리 누락 및 크래시 발생.
     * 기본 display로 폴백해 cleanup_task가 disp를 제대로 받도록 한다. */
    ...
    while(layer->draw_task_head) {
        ...
        task_dispatched = lv_draw_dispatch_layer(disp, layer);
