...
void lv_obj_remove_flag(lv_obj_t * obj, lv_obj_flag_t f)
{
    ...
    if(f & LV_OBJ_FLAG_HIDDEN) {
        ...
        /* obj가 화면(screen) 자체면 부모가 없어 NULL이 된다 (lv_obj_style.c:303과
         * 동일한 방어패턴) — password popup에서 active screen을 직접 HIDDEN
         * 해제할 때 NULL->layout_inv 역참조로 MemFault가 발생하던 문제 수정. */
        lv_obj_t * parent_for_hidden = lv_obj_get_parent(obj);
        if(parent_for_hidden) lv_obj_mark_layout_as_dirty(parent_for_hidden);
        lv_obj_mark_layout_as_dirty(obj);
    }

    if((was_on_layout != lv_obj_is_layout_positioned(obj)) || (f & (LV_OBJ_FLAG_LAYOUT_1 |  LV_OBJ_FLAG_LAYOUT_2))) {
        lv_obj_t * parent_for_layout = lv_obj_get_parent(obj);
        if(parent_for_layout) lv_obj_mark_layout_as_dirty(parent_for_layout);
    }
