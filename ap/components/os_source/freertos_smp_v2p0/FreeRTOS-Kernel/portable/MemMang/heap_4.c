...
static __attribute__((section(".psram.bss"))) free_record_type volatile s_psram_malloc_records[MALLOC_RECORD_MAX];
static __attribute__((section(".psram.bss"))) volatile uint32_t s_psram_malloc_record_index = 0;
...
/* 1: psram_malloc 실패 시 즉시 BK_ASSERT(0) → 덤프 + 강제 리부팅 (기존 동작).
 * 0: NULL만 반환 — 호출자가 이미 갖춘 fallback(bg_canvas_preload/
 *    popuppassword_bg_preload의 NULL 체크, LVGL 자체의 decode 실패 skip
 *    처리)이 정상 작동해서 "이미지 하나 안 보임/대체 경로 사용" 수준으로
 *    완화됨. PSRAM fragmentation으로 인한 malloc 실패를 리부팅 대신
 *    소프트하게 넘기기 위한 컴파일 옵션. 단, NULL 체크 없이 malloc 결과를
 *    바로 쓰는 코드가 있다면 그쪽에서 대신 크래시가 날 수 있음. */
#define PSRAM_MALLOC_ASSERT_ON_FAIL   0
...
#endif
{
	...
	if (pvReturn == NULL) {
#if CONFIG_MALLOC_STATIS || CONFIG_MEM_DEBUG
		BK_DUMP_OUT("[PSRAM] malloc FAILED: want=%u B  caller=%s:%d\r\n",
		            (unsigned)xWantedSize,
		            call_func_name ? call_func_name : "?", line);
#else
		BK_DUMP_OUT("[PSRAM] malloc FAILED: want=%u B\r\n", (unsigned)xWantedSize);
#endif
#if CONFIG_DEBUG_VERSION && PSRAM_MALLOC_ASSERT_ON_FAIL
		BK_ASSERT(0);
#endif
	}
...
#define DTCM_START_ADDRESS ((uint32_t)&__dtcm_cpu0_start__)

extern unsigned char __dtcm_cpu0_end__;
#define DTCM_END_ADDRESS ((uint32_t)&__dtcm_cpu0_end__)

extern unsigned char __itcm_cpu0_start__;
#define ITCM_START_ADDRESS ((uint32_t)&__itcm_cpu0_start__)

extern unsigned char __itcm_cpu0_end__;
#define ITCM_END_ADDRESS ((uint32_t)&__itcm_cpu0_end__)
