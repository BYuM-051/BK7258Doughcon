...
#define SUPPORT_100K   /* DS1338Z write fails at 400kHz; slower timing required */
...
{
	/* Propagate NACK: return BK_FAIL if device did not acknowledge */
	return I2cMemWrite(id, mem_param) ? BK_OK : BK_FAIL;
}

bk_err_t bk_i2c_memory_read_v2(i2c_id_t id, const i2c_mem_param_t *mem_param)
{
	/* Propagate NACK: return BK_FAIL if device did not acknowledge */
	return I2cMemRead(id, mem_param) ? BK_OK : BK_FAIL;
}
