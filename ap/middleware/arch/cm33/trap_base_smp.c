...
void stack_mem_dump_val(uint32_t stack_top, uint32_t stack_bottom)
{
    uint32_t *addr= (uint32_t *)stack_top;
    for(;stack_top < stack_bottom;)
    {
        BK_DUMP_OUT("addr:%x=%x\r\n", addr, addr[0]);
        addr++;
        stack_top += 4;
    }
}
...
static void arch_dump_cpu_registers(uint32_t mcause, SAVED_CONTEXT *context) {
    ...
    stack_mem_dump_val(context->sp, context->sp + 0x100);
}
